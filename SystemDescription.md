...
- **Multi-Sélection & Groupement Solidaire (Node Grouping & Solid Blocks)** : Sélection de plusieurs nœuds simultanément par rectangle de sélection. Bouton `Grouper en bloc 📦` pour lier la position des nœuds entre eux (`group_id`) afin qu'ils se déplacent en bloc solidaire sur le canevas, et `Dissoudre le groupe 🔓` pour débloquer.
- **Auto-Arrangement Sélectif** : Possibilité d'appliquer un auto-arrangement (Horizontale LR, Verticale TB, Grille GRID) **uniquement aux nœuds sélectionnés**, sans impacter la position des autres nœuds sur le canevas.
- **Suppression Définitive Synchronisée & Persistance Inter-Vues** : Suppression unitaire ou groupée (touches `Delete`/`Backspace` ou barre d'actions) répercutée de façon transactionnelle dans Neo4j. Les nœuds supprimés ne réapparaissent jamais lors du basculement entre la Carte, le Kanban Jira, le Gantt ou la Vue Liste.
...
- **Importation de Cartes Heuristiques XMind** : Bouton **`Importer XMind 📥`** ouvrant une modal dédiée au glisser-déposer de fichiers `.xmind` (XMind ZEN, 2020+, XMind 8 et JSON). Extraction automatique de l'arborescence, des notes dans la description, des marqueurs d'avancement/priorité et création des liaisons `CONTIENT_ETAPE` et `LIE_A` avec mise en page spatiale automatique.
- **Déplacement et persistance** : Positionnement libre des nœuds avec sauvegarde automatique des coordonnées (`pos_x`, `pos_y`).
- **Auto-arrangement global** : Réorganisation automatique globale du canevas via le menu *Auto-Arranger* (dispositions : Mindmap Horizontale, Arbre Vertical, Grille Matrice).
