...
try:
    ...
    cmd_body = (
        ...
        f"fi\n"
        f"echo '[NAS] Arrêt et suppression des anciens conteneurs Sentinel...'\n"
        f"docker compose down --volumes --remove-orphans 2>/dev/null || true\n"
        f"docker rm -f sentinel_neo4j sentinel_backend sentinel_discord_bot sentinel_frontend sentinel_nginx 2>/dev/null || true\n"
        f"docker rm -f $(docker ps -a --filter name=sentinel -q) 2>/dev/null || true\n"
