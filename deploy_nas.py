...
try:
    ...
    cmd_body = (
        ...
        f"fi\n"
        f"if docker ps --format '{{{{.Names}}}}' | grep -q sentinel_neo4j; then\n"
        f"  echo '[NAS] Sauvegarde pré-déploiement de Neo4j...'\n"
        f"  mkdir -p ./backups 2>/dev/null || true\n"
        f"  docker exec sentinel_neo4j cypher-shell -u neo4j -p '{config.get('neo4j_password', 'sentinel123')}' \"MATCH (n) OPTIONAL MATCH (n)-[r]->(m) RETURN count(n), count(r);\" > ./backups/pre_deploy_snapshot.log 2>/dev/null || true\n"
        f"fi\n"
        f"echo '[NAS] Arrêt propre des conteneurs (conservation des volumes de données)...'\n"
